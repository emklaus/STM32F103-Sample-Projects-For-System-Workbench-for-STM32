...
uint32_t VCP_Tx_ptr_in;
uint32_t VCP_Tx_ptr_out;
uint32_t VCP_Tx_length;
uint32_t VCP_Rx_ptr_in;
uint32_t VCP_Rx_ptr_out;
uint32_t VCP_Rx_length;
...
/*******************************************************************************
* Function Name  : USB_To_VCP_Receive_Buffer
* Description    : append the received data from USB to the UART 0.
...
void USB_To_VCP_Receive_Buffer(uint8_t* data_buffer, uint8_t Nb_bytes)
{
  ...
  {
	VCP_Rx_Buffer[VCP_Rx_ptr_in]= *(data_buffer + i);
	VCP_Rx_ptr_in++;
	
	if(VCP_Rx_ptr_in == VCP_RX_DATA_SIZE)  // Overflow
	    {
		  VCP_Rx_ptr_in = 0;
		  VCP_Rx_length = 0;
	    }
...
void Handle_USBAsynchXfer (void)
{
  ...
  if(USB_Tx_State != 1)
  {
    ...
    USB_Tx_State = 1; 
    UserToPMABufferCopy(&VCP_Tx_Buffer[USB_Tx_ptr], ENDP1_TXADDR, USB_Tx_length);
